...
def thread_process(num):
    ...
    if len(out)==0:
        ...
        for i in range(5):
            ...
            stop_CV=0
            try:
                subprocess.check_call(shlex.split('./Model_Train/call_DPR.sh'+' '+CV_file_dir+' '+str(args.dpr)+' '+TargetID_CV+' '+CV_output))
        ...
        if sum(k_fold_R2)/5 < 0.01:
            ...
            stop_DPR=0
            try:
                subprocess.check_call(shlex.split('./Model_Train/call_DPR.sh'+' '+file_dir+' '+str(args.dpr)+' '+TargetID+' '+args.out_prefix))
            except subprocess.CalledProcessError as err:
                stop_DPR=1
