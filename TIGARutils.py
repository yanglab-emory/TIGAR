...
def error_handler(func):
    ...
    def wrapper(num, *args, **kwargs):
        try:
            ...
            print(
                "Caught an exception for num={}:\n  {}: {}\nTraceback:\n{}".format(
                    num, e_type, e, e_tracebk
                )
            )
...
def ped_startup(ped_path, pedinfo_path):
    Asso_Info = pd.read_csv(pedinfo_path, sep="\t", header=None, names=["Ind", "Var"])
    ...
    if len(missing_cols):
        print(
            "WARNING: The following phenotypes and/or covariates given in PED_info are missing in the PED file: "
            + ", ".join(missing_cols)
            + "\n"
        )
...
def sampleid_startup(
    ...
    **kwargs,
    ...
    if ped_path and pedinfo_path:
        ped_sampleid, ped_cols, n_pheno, pheno, cov = ped_startup(
            ped_path, pedinfo_path
        )
    ...
    if sampleid_path:
        ...
        spec_sampleids = pd.read_csv(sampleid_path, sep="\t", header=None)[
            0
        ].drop_duplicates()
...
def bgw_weight_file_info(
    w_path, chrm, weight_threshold=0, add_cols=[], drop_cols=["ID"], **kwargs
):
    info_dict = get_cols_dtype(
    ...
    **kwargs,
    ...
    if chrm is not None:
        try:
            ...
            Gene = pd.concat([x[x["CHROM"] == chrm] for x in Gene_chunks]).reset_index(
                drop=True
            )
        except:
            Gene_chunks = pd.read_csv(
                path, sep="\t", iterator=True, chunksize=10000, usecols=cols
            )
            Gene = (
                pd.concat([x[x["CHROM"] == chrm] for x in Gene_chunks])
        ...
        if Gene.empty:
            raise SystemExit(
                "There are no valid gene expression/annotation data for chromosome "
                + chrm
                + ".\n"
            )
    else:
        try:
            Gene = pd.read_csv(path, sep="\t", usecols=cols, dtype=dtype)
        except:
            Gene = (
                pd.read_csv(path, sep="\t", header=None, usecols=cols)
                .reset_index(drop=True)
                .astype(dtype)
            )
            # Gene.columns = [cols[i] for i in Gene.columns]
    ...
    **kwargs,
    ...
    if genofile_type == "weight":
        ...
        if weight_threshold:
            ...
            df = df[operator.gt(np.abs(df["ES"]), weight_threshold)].reset_index(
                drop=True
            )

            if df.empty and raise_error:
                print(
                    "No test SNPs with cis-eQTL weights with magnitude that exceeds specified weight threshold for TargetID: "
                    + target
                    + ".\n"
                )
                raise NoTargetDataError
...
def tabix_query_files(
    start, end, chrm, geno_path=None, gwas_path=None, w_path=None, z_path=None, **kwargs
):
    """"""
    paths = [
...
def get_header(path, out="tuple", zipped=False, rename={}):
    ...
    # decode bytes, use pandas to read in, rename columns from dictionary
    header = pd.read_csv(
        path, sep="\t", header=0, compression=compress_type, low_memory=False, nrows=0
    ).rename(columns=rename)
...
def gwas_cols_dtype(file_cols, ind_namekey=True, **kwargs):
    return get_cols_dtype(
        file_cols,
        cols=["CHROM", "POS", "REF", "ALT", "BETA", "SE"],
        ind_namekey=ind_namekey,
    )
...
def get_ld_cols(path):
    ...
    file_cols = tuple(
        pd.read_csv(
            path, sep="\t", header=0, compression="gzip", low_memory=False, nrows=0
        ).rename(
            columns={
                "#snpID": "snpID",
                "#ID": "snpID",
    ...
    try:
        ...
        while n:
            try:
                ...
                cov_data = pd.concat(
                    [
                        get_ld_regions_data(regs_str, *regs_args)
                        for regs_str in regs_str_lst
                    ]
                )
            except OSError:
                n -= 500
...
def flip_snpIDs(snpIDs):
    return np.array(
        [":".join([y[0], y[1], y[3], y[2]]) for y in [x.split(":") for x in snpIDs]]
    )
...
def reformat_sample_vals(df: pd.DataFrame, data_format, sampleID):
    ...
    df = pd.concat(
        [df.drop(columns=sampleID), pd.DataFrame(vals, columns=sampleID)], axis=1
    )
    return df
...
def handle_missing(
    df: pd.DataFrame, sampleID, missing_rate, filter=True, op=operator.le
):
    df = df.copy()
    ...
    MissingRate = pd.to_numeric(
        pd.Series(
            np.apply_along_axis(
                lambda x: np.count_nonzero(np.isnan(x)) / x.size, 1, vals
            ),
            name="missing_rate",
        ),
        downcast="float",
    ...
    df = pd.concat(
        [df.drop(columns=sampleID), pd.DataFrame(vals, columns=sampleID), MissingRate],
        axis=1,
    )
...
def row_maf_impute(x):
    # calculate MAF
    MAF_val = np.array(
        np.nansum(x) / (2 * (x.size - np.count_nonzero(np.isnan(x)))), dtype=x.dtype
    )
...
def calc_maf(
    df: pd.DataFrame, sampleID, maf, filter=True, op=operator.gt, filter_bid=False
):
    df = df.reset_index(drop=True).copy()
    vals = df[sampleID].values
    ...
    df = pd.concat(
        [
            ...
            pd.DataFrame(sample_MAF, columns=[*sampleID, "MAF"]),
        ],
        axis=1,
    )
...
def calc_p_hwe(df: pd.DataFrame, sampleID, pval, filter=True, op=operator.gt):
    ...
    HWE = pd.to_numeric(
        pd.Series(
            np.apply_along_axis(
                lambda x: prep_p_HWE(x[np.where(~np.isnan(x))]), 1, vals
            ),
            name="p_HWE",
        ),
        downcast="float",
    ...
    # re combine dataframe
    df = pd.concat(
        [df.drop(columns=sampleID), pd.DataFrame(vals, columns=sampleID), HWE], axis=1
    )
...
def HWE(obs_hets, obs_hom1, obs_hom2):
    if obs_hom1 < 0 or obs_hom2 < 0 or obs_hets < 0:
        raise Exception(
            "FATAL ERROR - SNP-HWE: Current genotype configuration (%s  %s %s) includes negative count"
