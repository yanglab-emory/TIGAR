...
def genofile_cols_dtype(file_cols, genofile_type, sampleid):
	return get_cols_dtype(file_cols, cols=['CHROM','POS','REF','ALT'],
		sampleid=sampleid, genofile_type=genofile_type)

def weight_cols_dtype(file_cols, add_cols=[], drop_cols=[], get_id=True):
	return get_cols_dtype(file_cols, cols=['CHROM','POS','REF','ALT','TargetID','ES'], 
		add_cols=add_cols, drop_cols=drop_cols, get_id=get_id)

def zscore_cols_dtype(file_cols):
	return get_cols_dtype(file_cols, cols=['CHROM','POS','REF','ALT','Zscore'])

def gwas_cols_dtype(file_cols):
	return get_cols_dtype(file_cols, cols=['CHROM','POS','REF','ALT','BETA','SE'])

def MCOV_cols_dtype(file_cols, add_cols=[], drop_cols=[], get_id=True):
	return get_cols_dtype(file_cols, cols=['CHROM','POS','REF','ALT','COV'], 
		add_cols=add_cols, drop_cols=drop_cols, get_id=get_id)
